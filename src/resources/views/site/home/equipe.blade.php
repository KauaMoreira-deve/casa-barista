<section class="equipe">
    <header class="parallax-padrao  wow animate__animated animate__fadeInUp">
        <h2>Quem Somos</h2>
        <h3>O café é feito por pessoas</h3>
    </header>

    <div class="site cardEquipe  wow animate__animated animate__fadeInUp">
        <article>
            <img src="assets/barista1.png" alt="Lucas Ribeiro - Barista Especialista">
            <h4>Lucas Ribeiro</h4>
            <h5>Barista Especialista</h5>
        </article>

        <article>
            <img src="assets/barista2.png" alt="Mariana Alves - Mestre de Torr">
            <h4>Mariana Alves</h4>
            <h5>Mestre de Torra</h5>
        </article>

        <article>
            <img src="assets/barista3.png" alt="Renato Silva - Atendimento e Experiência do Cliente">
            <h4>Renato Silva</h4>
            <h5>Atendimento e Experiência do Cliente</h5>
        </article>
    </div>

</section>